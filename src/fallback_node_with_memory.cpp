...
FallbackNodeWithMemory::FallbackNodeWithMemory(std::string name)
...
ReturnStatus FallbackNodeWithMemory::Tick() {
    DEBUG_STDOUT(
            get_name() << " ticked, memory counter: " << current_child_idx_);
        ...
        */
        ...
        if (child_status_ != FAILURE) {
            ...
            DEBUG_STDOUT("the status of: " << get_name() << " becomes "
                                           << child_status_);
            if (child_status_ == SUCCESS && (reset_policy_ == ON_SUCCESS
                                                 || reset_policy_ ==
...
int FallbackNodeWithMemory::DrawType() {
    return SELECTORSTAR;
}
